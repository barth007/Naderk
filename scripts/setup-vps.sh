...
echo ""
echo "2. Pin the per-environment variables (do this once per deploy directory):"
echo ""
echo "   printf 'ENVIRONMENT=production\\nBACKEND_PORT=8000\\nFRONTEND_PORT=3000\\n' \\"
echo "     > /var/www/naderk/.env"
echo "   printf 'ENVIRONMENT=dev\\nBACKEND_PORT=8001\\nFRONTEND_PORT=3001\\n' \\"
echo "     > /var/www/naderk-dev/.env"
echo ""
echo "   Docker Compose reads .env automatically from the directory you run in."
echo "   The deploy workflow exports all three, but MANUAL commands do not — and"
echo "   the compose defaults are the PRODUCTION values (.env.production, 8000,"
echo "   3000). So a manual 'docker compose up' on the dev box will either fail"
echo "   with 'env file not found' or try to bind production ports and collide."
