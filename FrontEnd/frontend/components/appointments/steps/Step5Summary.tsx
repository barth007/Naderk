...
export default function Step5Summary() {
  ...
  const numericFee = parseFloat(consultationFee);
  const requiresPayment = numericFee > 0 && !isConsultationValid;

  const handleConfirm = async () => {
    if (!service || !doctor || !date || !time) return;
    setErrorMsg(null);

    if (!requiresPayment) {
      // Free booking — create appointment directly
      createAppointmentMutation.mutate(
        { service_id: service.id, doctor_id: doctor.id, date, time, appointment_type: appointmentType, notes },
        {
          onSuccess: () => nextStep(),
          ...
          <div className="flex flex-col md:flex-row gap-8">
            <div className="flex-1 space-y-6">
              <div>
                ...
                <p className="text-sm text-gray-500">{appointmentType === 'PHYSICAL' ? 'In-Person Visit' : 'Telehealth Video Call'}</p>
              </div>
              <div>
