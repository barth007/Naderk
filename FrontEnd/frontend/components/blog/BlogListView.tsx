...
/**
 * The article list, shared by the public site and the dashboard.
 *
 * The dashboard's "View All" pointed at /blog — the public marketing site —
 * so a logged-in patient reading an article was dropped out of the dashboard
 * shell entirely. `basePath` keeps article links inside whichever section the
 * reader is already in.
 */
export function BlogListView({ basePath = '/blog', heading = true }: { basePath?: string; heading?: boolean }) {
  ...
  return (
    <div className="w-full pb-10">
      {/* The public site wants a full-width hero; inside the dashboard the
          page already has a heading, so it is suppressed. */}
      {heading && (
      ...
      <div className={heading ? 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12' : 'py-2'}>
        ...
        {isLoadingBlogs ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {blogs.map((blog) => (
              <Link href={`${basePath}/${blog.slug}`} key={blog.id} className="group flex flex-col bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300">
          ...
          <div className="text-center py-20 bg-white rounded-xl border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-500">We couldn&rsquo;t find any healthcare articles for this category.</p>
            <Button 
